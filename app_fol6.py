...
st.header("Карта мусорных контейнеров")
...
html1 = '<a href=https://waico.ru/ko_meta/geo_cam_rai.htm>Ссылка на аналитику и статистику</a>'
html2 = '<a href=file:///D:/Programming/Hackatons/ZP/gui4/geo_cam_rai1.htm>Ссылка на видео</a>'
#htm = '<a href=file:///D:/Programming/Hackatons/ZP/gui4/geo_cam_rai1.htm>Link 1</a>'
...
popup=[html1, html2], tooltip=tooltip, 
...
if st.button("Кликните на кнопку, чтобы увидеть аналитику и видео по выбранной камере"):
    video_file = open("4.mp4", 'rb')
...
#webbrowser.open('geo_cam_rai1.htm')


uploaded_files = st.file_uploader("Добавить стрим поток", accept_multiple_files=True)
for ind,file in enumerate(uploaded_files):
    ...
    opt = SimpleNamespace(agnostic_nms=False, augment=False, classes=None, conf_thres=0.5, device='CPU', dnn=False, exist_ok=False,\
                                   ...
                                   max_det=1000, name='exp', nosave=False, project=f'runs/stream{ind}', save_conf=False, save_crop=False,\
                                   save_txt=False,source=file.name, update=False, view_img=False, visualize=False, weights=['best.pt'])


    det.main(opt)
    imgg = Image.open(f'runs/stream{ind}/exp/{file.name}')
    st.image(imgg)


if st.button("Предсказание для камер, по которым обнаружены превышения"):
